...
def prepare_prithvi_frame(frame: np.ndarray) -> np.ndarray:
    ...
    if frame.ndim != 3 or frame.shape[0] != len(ALL_BANDS):
        raise ValueError(
            f"Expected frame [{len(ALL_BANDS)},H,W], "
    ...
    # dataMask is the final band in the 9-band stack.
    data_mask = frame[8] > 0

    x = (
        x - PRITHVI_MEAN[:, None, None]
    ) / PRITHVI_STD[:, None, None]
...
def prepare_prithvi_tensor(
    ...
    if len(paths) != 3:
        raise ValueError(
            "Exactly three frames are required for the v1 MVP."
        )

    raw_frames = [
        read_process_tiff(path)
        for path in paths
    ]
    ...
    for frame in raw_frames:
        if frame.shape[-2:] != (expected_size, expected_size):
            raise ValueError(
                ...
                f"got {frame.shape[-2:]}. "
                "Request aligned patches at the model size "
                "rather than resizing here."
            )

    normalized = [
        prepare_prithvi_frame(frame)
        for frame in raw_frames
    ]

    # [C, T, H, W]
    cube = np.stack(normalized, axis=1)
...
def spectral_indices(frame: np.ndarray) -> dict:
    ...
    # Sentinel-2 band positions in our stack.
    b02 = frame[0] / 10000.0
    b04 = frame[2] / 10000.0
    b11 = frame[4] / 10000.0
    b08 = frame[6] / 10000.0
    ...
    def masked_mean(x: np.ndarray) -> float:
        values = x[
            valid & np.isfinite(x)
        ]
